...
public class RowOfMSSes {
    ...
    public static void main(String[] args) throws PetriObjectModelException, ExecutionException, InterruptedException {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            PetriObjectModel model = createRowOfMSSesWithGenerator(petriObjectsAmount * (i + 1), mSSesInOneObjectAmount, channelsInOneMSS, generatorDelayGenerator, mSSDelayGenerator);
...
//        System.out.println(statistics.getPetriObjectStatisticsByPetriObjectId(petriObjectsAmount - 1).getPlaceStatisticsByPlaceId((petriObjectsAmount - 1) * mSSesInOneObjectAmount * 2 + 1));
    }

    private static PetriObjectModel createRowOfMSSesWithGenerator(
            int objectsAmount,
        ...
        petriObjects.add(createGenerator(generatorDelayGenerator));
        for (int i = 1; i < objectsAmount; ++i) {

            PetriObject mSS = createMSSGroup(i, mSSesInOneObjectAmount, channels, mSSDelayGenerator, i + 1 == objectsAmount);

            petriObjects.add(mSS);

            petriObjects.get(i - 1).getTransitionById((i - 1) * mSSesInOneObjectAmount).addArcTo(mSS.getPlaceById((i - 1) * (mSSesInOneObjectAmount * 2) + 1));
        }

        return new PetriObjectModel(petriObjects);
    }

    private static PetriObject createGenerator(DelayGenerator generatorDelayGenerator) throws PetriObjectModelException {
        ...
        return new PetriObject(0, "Generator", new ArrayList<Transition>(){{add(generator);}});
    }

    private static PetriObject createMSSGroup(int mSSGroupId, int mSSesInOneObjectAmount, int channels, DelayGenerator mSSDelayGenerator, boolean lastGroup) throws PetriObjectModelException {
        ...
        for (int i = 0; i < mSSesInOneObjectAmount; ++i) {
            ...
            boolean lastMSS = lastGroup && (i + 1 == mSSesInOneObjectAmount);

            Transition mSS = createMSS((mSSGroupId - 1) * mSSesInOneObjectAmount + i + 1, channels, mSSDelayGenerator, prevTransition, lastMSS);
    ...
    private static Transition createMSS(int mSSId, int channels, DelayGenerator mSSDelayGenerator, Transition prevTransition, boolean lastMSS) {
        ...
        if (lastMSS) {
            Place end = new Place(mSSId * 2 + 1, "End", false);
            mSS.addArcTo(end);
        }
